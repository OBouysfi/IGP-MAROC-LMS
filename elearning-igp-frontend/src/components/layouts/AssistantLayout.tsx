...
export default function AssistantLayout({ children }: AssistantLayoutProps) {
  ...
  const menuItems = [
    ...
    { name: 'Gestion Absences', icon: UserCheck, path: '/assistant/absences' },
    { name: 'Justificatifs', icon: FileText, path: '/assistant/justifications' },
    { name: 'Emplois du Temps', icon: Calendar, path: '/assistant/schedules' },
    { name: 'Rapports', icon: ClipboardList, path: '/assistant/reports' },
    { name: 'Retards', icon: Clock, path: '/assistant/delays' },
  ];

  const notifications = [
    { id: 1, message: 'Nouveau justificatif à valider', time: 'Il y a 10 min', read: false },
    { id: 2, message: '5 absences non justifiées cette semaine', time: 'Il y a 1h', read: false },
            ...
            {sidebarOpen ? (
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 bg-white rounded-lg flex items-center justify-center">
                  <span className="text-[#C1272D] font-bold text-lg">IGP</span>
                </div>
                <div>
                  <h1 className="font-bold text-lg">IGP Maroc</h1>
                  <p className="text-xs text-red-200">Scolarité</p>
                </div>
              </div>
            ) : (
            ...
            return (
              <Link
                ...
                className={`flex items-center gap-3 px-4 py-3 rounded-lg transition-all duration-200 ${
                  isActive
                    ? 'bg-white text-[#C1272D] shadow-lg'
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <h2 className="text-xl font-bold text-gray-800">
                  {menuItems.find(item => item.path === pathname)?.name || 'Scolarité'}
                </h2>
                <p className="text-sm text-gray-500">Gestion des absences et présences</p>
              </div>
            </div>
